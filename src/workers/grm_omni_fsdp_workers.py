...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for prefix in ["revised_response_a", "revised_response_b", 'raw_response_a', 'raw_response_b']:
            for i in range(data.batch.batch_size[0]):
                chat: list = data.non_tensor_batch['markov_messages'][i][prefix]
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        if data.non_tensor_batch['interaction_mode'][0] == "meta_reward":
            ...
            with self.ulysses_sharding_manager:
                ...
                length = len(scores) // 2
                for s1, s2, r1, r2, pred, gt in zip(scores[:length//2], scores[length//2:length], scores[length:length + length//2], scores[length + length//2:], data.non_tensor_batch['markov_judge'], data.non_tensor_batch['ground_truth']):
                    
                    if pred[0] == 0 and s1 > s2:
                        s_meta.append(s2 - r2)
                    elif pred[0] == 0 and s1 < s2:
                        s_meta.append(s2 - r2)
                    elif pred[0] == 1 and s1 < s2:
                        s_meta.append(s1 - r1)
                    elif pred[0] == 1 and s1 > s2:
                        s_meta.append(s1 - r1)
                    else:
                        s_meta.append(torch.tensor(-999., device=get_device_id()))
                    
                    if pred[0] == gt:
                        collection_count.append(1)
                        ground_truth.append(torch.tensor(1.0, device=get_device_id()))
                    else:
                        collection_count.append(0)
                        ground_truth.append(torch.tensor(0.0, device=get_device_id()))
                ...
                print(f"[debug] dis_count============>: {dis_count/count}")
                print(f"[debug] grm_count============>: {grm_count/count}")
                print(f"[debug] count============>: {Counter(collection_count)}")

                scores = torch.stack(s_meta).to(get_device_id())
                ground_truth_scores = torch.stack(ground_truth).to(get_device_id())

                token_level_scores = self._expand_to_token_level(data, scores)
                token_level_gt_scores = self._expand_to_token_level(data, ground_truth_scores)
                # Note that this is only the scores, may not be the final rewards used to train RL
                output = DataProto.from_dict(tensors={
                    "rm_scores": token_level_gt_scores,
                    "rank_scores": token_level_scores,
        ...
        elif data.non_tensor_batch['interaction_mode'][0] == "grm":
            with self.ulysses_sharding_manager:
                collection_count = []
                ground_truth = []
                count = 0
                grm_count = 0
                for pred, gt in zip(data.non_tensor_batch['markov_judge'], data.non_tensor_batch['ground_truth']):
        ...
        elif data.non_tensor_batch['interaction_mode'][0] == "meta_reward_golden":
            ...
            with self.ulysses_sharding_manager:
                ...
                for uid, judge, gt, access_index, gt_list in zip(data.non_tensor_batch['uid'], data.non_tensor_batch['markov_judge'], data.non_tensor_batch['ground_truth'], data.non_tensor_batch['access_index'], data.non_tensor_batch['ground_truth_list']):

                    
                    if judge[0][0] == 1:
                        collection_count.append(1)
                        ground_truth.append(torch.tensor(judge[0][0],device=get_device_id()))
                    elif judge[0][0] == 0.5:
                        ground_truth.append(torch.tensor(judge[0][0],device=get_device_id()))
                        collection_count.append(0.5)
                    else:
                        ground_truth.append(torch.tensor(0.0,device=get_device_id()))
                        collection_count.append(0.0)
                ...
                scale_judge = []
                pred_judge = []
                for uid, judge, gt, acccess_idx, g_list in zip(data.non_tensor_batch['uid'], data.non_tensor_batch['markov_judge'], data.non_tensor_batch['ground_truth'], data.non_tensor_batch['access_index'], data.non_tensor_batch['ground_truth_list']):
                    
                    if judge[0][0] == 1 and int(g_list[acccess_idx]):
                        scale_judge.append(int(judge[0][-1]) / int(judge[0][-2]))
                        pred_judge.append(int(g_list[acccess_idx]))
                    else:
                        scale_judge.append(0.0)
                        pred_judge.append(0.0)
                
                scale_judge = torch.stack([torch.tensor(x) for x in scale_judge]).to(get_device_id())
                pred_judge = torch.stack([torch.tensor(x) for x in pred_judge]).to(get_device_id())
                output = DataProto.from_dict(tensors={
                    "rm_scores": token_level_gt_scores,
                    "scale_judge": scale_judge,
                    "pred_judge": pred_judge,
                    "ground_truth_scores": ground_truth_scores,
                })

        else:
            raise Exception(f"Not Support This Interaction {data.non_tensor_batch['interaction_mode']}")
